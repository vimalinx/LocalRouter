...
jq -e '.success == true and .data.admin_auth_enabled == false and .data.billing == "usage-accounting" and .data.oauth == "external-maintainer" and .data.update.enabled == false and .data.update.status == "disabled"' "$smoke_root/summary.json" >/dev/null
curl --fail --silent --show-error --request POST "http://127.0.0.1:${smoke_port}/local/api/update/check" >"$smoke_root/update.json"
jq -e '.success == true and .data.enabled == false and .data.status == "disabled"' "$smoke_root/update.json" >/dev/null
