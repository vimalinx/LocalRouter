...
Generated `request_example` values demonstrate shape only. They are never evidence that a model, resource ID, price, or entitlement is currently available. When the runtime publishes `dynamic_inputs`, Agents must resolve those fields first. For a model input, prefer the advertised `source_operation_key`/`source_call_url`, extract the published IDs, and choose one exact result. When a provider has no model-catalog operation, the Pack may instead publish a reviewed explicit request-schema string `enum`; the runtime identifies it as `source_kind=request-schema-enum`, and Agents must refresh discovery when the contract digest changes. Do not copy a stale `request_example.model` or invent a model outside either source.
