...
it('shows services as a searchable list with status and remaining allowance', async () => {
  const other = { ...item, service: 'image', name: '图像服务', rule: { ...rule, enabled: true, limit: 10 }, spent: 2, reserved: 3, remaining: 5, pending: [{ id: 'receipt-1', operation: 'create', token_id: 2, amount: 3 }] }
  vi.mocked(adminRequest).mockResolvedValue([item, other])
  const user = userEvent.setup(); render(<ServiceAllowances adminToken='' />)
  expect(await screen.findByRole('button', { name: '测试服务 自主额度' })).toHaveAttribute('aria-current', 'page')
  expect(screen.getByRole('button', { name: '图像服务 自主额度' })).toHaveTextContent('额度内自主')
  expect(screen.getByText('剩余 5 次 / 10 次')).toBeInTheDocument()
  expect(screen.getByText('1 笔费用待核对')).toBeInTheDocument()
  ...
  expect(screen.getByRole('checkbox', { name: '启用此服务的自主使用限制' })).toBeChecked()
  expect(adminRequest).toHaveBeenCalledTimes(1)
})
...
it('saves multiple service limits atomically and preserves their enabled states', async () => {
  ...
  const user = userEvent.setup(); render(<ServiceAllowances adminToken='' />)
  await user.click(await screen.findByRole('checkbox', { name: '选择全部筛选服务' }))
