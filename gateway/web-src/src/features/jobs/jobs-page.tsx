...
export function JobsPage(props: { jobs: WorkflowJob[]; events: ProtocolEvent[] }) {
  return <div className='space-y-8'>
    ...
    <section aria-labelledby='events-title'><div className='flex items-center justify-between border-b pb-2'><h2 id='events-title' className='text-sm font-semibold'>Protocol Events</h2><span className='text-xs text-muted-foreground'>{props.events.length}</span></div>{props.events.length ? <Table><TableHeader><TableRow><TableHead>时间</TableHead><TableHead>入口</TableHead><TableHead>操作</TableHead><TableHead>结果</TableHead><TableHead>延迟</TableHead><TableHead>后端 / 池引用</TableHead></TableRow></TableHeader><TableBody>{props.events.map((event) => <TableRow key={event.id}><TableCell className='whitespace-nowrap text-xs text-muted-foreground'>{formatTimestamp(event.created_at)}</TableCell><TableCell><Badge variant='outline'>{event.surface}</Badge></TableCell><TableCell><p className='text-xs font-medium'>{event.protocol_id || 'model'} · {event.operation_id || event.method}</p><code className='block max-w-72 truncate text-[10px] text-muted-foreground'>{event.path}</code></TableCell><TableCell><Badge variant={event.status >= 200 && event.status < 400 ? 'success' : 'destructive'}>{event.status}</Badge></TableCell><TableCell className='tabular-nums'>{event.latency_ms} ms</TableCell><TableCell>{event.target ? <p className='text-xs font-medium'>{event.target}</p> : null}<code className='text-[10px]'>{event.credential_ref || '—'}</code></TableCell></TableRow>)}</TableBody></Table> : <EmptyState icon={Activity} title='暂无协议事件' description='Protocol Pack、Workflow 与 MCP 调用会写入脱敏事件流。' />}</section>
  </div>
}
